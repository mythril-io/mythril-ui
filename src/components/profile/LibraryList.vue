            ...
            <div class="min-w-0 flex-1 flex items-center">
              <div class="flex-shrink-0">
                <div class="overflow-hidden inline-block h-12 w-12 rounded-md border border-gray-100">
                  <img class="group-hover:scale-105 transform duration-150 ease-in-out" :src="$store.state.gameIconURL + entry.game.icon" :alt="entry.game.title" />
                </div>
              </div>
              <div class="min-w-0 flex-1 px-4 md:grid md:grid-cols-2 md:gap-4 items-center">
                <div>
                  <div class="text-sm md:text-md leading-5 font-bold text-primary truncate">{{ entry.game.title }}</div>
