  ...
  <div>
    <div>
      ...
      <div class="hidden sm:block">
        <nav class="flex">
          <router-link tag="a" :to="{ name: 'Forums' }" class="is-pill-tab ml-0">
            Recent
          </router-link>
          <router-link tag="a" :to="{ name: 'Games' }" class="is-pill-tab">
            Popular
          </router-link>
          <router-link tag="a" :to="{ name: 'Games' }" class="is-pill-tab">
            Likes
    ...
    <transition name="fade">
      <div v-if="!loading && data.items.length" class="bg-white border border-gray-200 sm:rounded-md mt-5">
        <ul>
          <li class="border-t border-gray-200 first:border-t-0" v-for="discussion in data.items" :key="discussion.id">
                ...
                <div class="min-w-0 flex-1 px-4 md:grid md:grid-cols-2 md:gap-4 items-center">
                  <div>
                    ...
                    <div class="mt-2 flex items-center">
                      <router-link :to="{name: 'Forums', params: { tag: tag.slug }}" :class="'tag first:ml-0 ml-2 ' + tag.colour" v-for="tag in discussion.tags" :key="tag.id">
                        {{ tag.name }}
                      </router-link>
                      <span class="tag ml-2" v-for="tag in discussion.games" :key="tag.id">
                        {{ tag.name }}
                      </span>

                      <span class="tag ml-2 bg-primary text-white">
                        Final Fantasy VII
                      </span>
                    </div>
                  </div>
