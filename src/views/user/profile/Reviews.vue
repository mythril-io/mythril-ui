<template>
  <div>
    ...
    <div v-else>
      ...
      <ReviewsList v-if="data.data && data.data.length" :data="data" :loading="loading" v-on:get-data="getData($event)"/>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    getData (page=1) {
      ...
      this.loading = true;
      return reviewService.getByUserAndPage(this.user.username, page).then(
        response => {
