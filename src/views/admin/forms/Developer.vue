        ...
        <div class="md:col-span-1">
          <div class="px-4 sm:px-0">
            <p class="mt-1 text-sm leading-5 text-gray-500">
              Please provide a developer name and country.
            </p>
          </div>
        </div>
...
export default {
  ...
  computed: {
      title () {
          if (this.$route.params.id) {
            return "Edit a Developer";
          } else {
            return "Create a Developer"
          }
      }
  },
  methods: {
    ...
    cancel() {
      this.$router.push({ name: 'AdminDevelopers' });
    },
    post (resource) {
      const { dispatch } = this.$store;
      developerService.post(resource).then(
        response => {
          this.$router.push({ name: 'AdminDevelopers' });
        },
        error => {
    ...
    put (resource) {
      const { dispatch } = this.$store;
      developerService.put(resource).then(
        response => {
          this.$router.push({ name: 'AdminDevelopers' });
  ...
  created () {
      if (this.$route.params.id) {
        this.loading = true;
        const { dispatch } = this.$store;
        developerService.get(this.$route.params.id).then(
          response => {
