        ...
        <div class="md:col-span-1">
          <div class="px-4 sm:px-0">
            <p class="mt-1 text-sm leading-5 text-gray-500">
              Please provide a genre name and acronym.
            </p>
          </div>
        </div>
              ...
              <div class="shadow sm:rounded-md sm:overflow-hidden">
                <div class="px-4 py-5 bg-white sm:p-6">
                  <div>
                    ...
                    <ValidationProvider rules="required" v-slot="{ errors }">
                      <div class="mt-1 flex rounded-md shadow-sm">
                        <input id="name" v-model="resource.name" class="form-input flex-1 block w-full rounded-none rounded-md transition duration-150 ease-in-out sm:text-sm sm:leading-5" placeholder="Role-Playing Game" />
                      </div>
                  ...
                  <div class="mt-6">
                    ...
                    <ValidationProvider rules="required" v-slot="{ errors }">
                      <div class="mt-1 flex rounded-md shadow-sm">
                        <input id="acronym" v-model="resource.acronym" class="form-input flex-1 block w-full rounded-none rounded-md transition duration-150 ease-in-out sm:text-sm sm:leading-5" placeholder="RPG" />
                      </div>
                      <span class="text-sm text-red-700">{{ errors[0] }}</span>
                    </ValidationProvider>
                  </div>
...
export default {
  ...
  methods: {
    ...
    post (resource) {
      ...
      genreService.post(resource).then(
        response => {
          this.$router.push({ name: 'AdminGenres' });
        },
        error => {
    ...
    put (resource) {
      const { dispatch } = this.$store;
      genreService.put(resource).then(
        response => {
          this.$router.push({ name: 'AdminGenres' });
  ...
  created () {
      if (this.$route.params.id) {
        this.loading = true;
        const { dispatch } = this.$store;
        genreService.get(this.$route.params.id).then(
          response => {
